...
async function loadData() {
  rooms = await fetch('data/config/rooms.json').then(r => r.json());
  tariff = await fetch('data/config/tariff.json').then(r => r.json());
  rules = await fetch('data/config/rules.json').then(r => r.json());
  restricted_periods = await fetch('data/config/restricted_periods.json').then(r => r.json());
}

// Generate calendar with vacant counts
...
async function startBooking() {
  ...
  createTable('guest-list', 'Member Details', 5);
  createTable('temp-guest-list', 'Temp Member Details', 5);
}
...
function calculateBooking() {
  ...
  occDiv.innerHTML = '<h3>Occupants per Room</h3>';
  selectedRooms.forEach(r => {
    let html = `<div class="room-occ">${r.room_no}: <select>`;
    for (let i = r.min_person; i <= r.max_person; i++) html += `<option>${i}</option>`;
    html += '</select></div>';
  ...
  // Validate occupants
  const occSelects = Array.from(occDiv.querySelectorAll('select'));
  const totalOcc = occSelects.reduce((sum, s) => sum + parseInt(s.value), 0);
  if (totalOcc < totalOccupants) return alert('Occupants exceed room capacity');
...
// Other functions unchanged (speak, admin, etc.)
